...
class SeatingArrangement_Facebook {
    ...
    int minOverallAwkwardness(int[] arr) {
        ...
        int N = arr.length;
        int item = 0 ;
        int index = 0, prevIndex=(index + N -1) % N, nextIndex = (index + 1) % N;
        nArr[index] = arr[item++];
        while (item < N){
            nArr[prevIndex--] = arr[item++];
            if(item < N){
                nArr[nextIndex++] = arr[item++];
            }
        }
    ...
    public static void main(String[] args) {
        new SeatingArrangement_Facebook().run();
    }
}
